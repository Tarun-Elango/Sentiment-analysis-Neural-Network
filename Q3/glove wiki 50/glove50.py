...
performance = open("performance for glove wiki 50.txt", "w")
...
def postEmb(post, x):  # takes in post number and post
    ...
    for i in range(len(post[x])):
        try:
            sumEmbedd = sumEmbedd + model[post[x][i]]
            totalCount = totalCount + 1
        except KeyError:
            pass  # print(post[x][i], 'not in model')

    if totalCount == 0:
        avg = np.array([0 for i in range(50)])
    else:
...
def createEmbVector(post):  # takes in tokenized data
    embVector = np.empty(shape=(len(post), 50))
    embVector.fill(0)
    for i in range(len(post)):
